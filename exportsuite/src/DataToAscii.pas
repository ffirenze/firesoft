...
// fucion modificado por Federico para que tome el Unicode
// ver tambien la clase que fue cambiada
// procedure WriteRecord; virtual; abstract; en DataExport
function TDataToAscii.Write(const Buffer; Count: Integer): Longint;
var
  len: integer;
  src, dst: TBytes;
begin
  ...
  SetLength(src, len);
  Move(Buffer, src[0], len);
  dst := TEncoding.Default.Convert(TEncoding.Unicode, TEncoding.ASCII, src, 0, len);

  Result := inherited Write(dst[0], Length(dst)-1);
end;
